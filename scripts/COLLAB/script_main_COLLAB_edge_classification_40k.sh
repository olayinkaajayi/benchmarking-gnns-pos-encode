...
dataset=OGBL-COLLAB
tmux new -s benchmark_COLLAB -d
tmux send-keys -t benchmark_COLLAB "cd codes/benchmarking-gnns" C-m
tmux send-keys -t benchmark_COLLAB "conda activate yinka_env" C-m

tmux send-keys -t benchmark_COLLAB "
CUDA_VISIBLE_DEVICES=$id python $code --dataset $dataset --gpu_id $id --seed $seed0 --config 'configs/COLLAB_edge_classification_GatedGCN_40k.json'
" C-m
