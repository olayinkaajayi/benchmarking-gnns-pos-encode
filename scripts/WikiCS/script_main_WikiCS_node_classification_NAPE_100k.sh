...
tmux new -s benchmark_NAPE -d
# tmux send-keys "source activate benchmark_gnn" C-m
tmux send-keys -t benchmark_NAPE "cd codes/benchmarking-gnns" C-m
tmux send-keys -t benchmark_NAPE "conda activate yinka_env" C-m
dataset=WikiCS

tmux send-keys -t benchmark_NAPE "
CUDA_VISIBLE_DEVICES=$id python $code --dataset $dataset --gpu_id $id --seed $seed0 --config 'configs/WikiCS_node_classification_MoNet_NAPE_100k.json'
" C-m
