...
config_file="$home/configs/WikiCS_node_classification_MoNet_NAPE_100k.json"
exp_name="eta"
# "pos_enc_name": "checkpoint_scaledNAPE_cpu.pt"
...
function run_experiments {
  ...
  config_file_mod="$home/scripts/WikiCS/Experiment/modified_config_$eta.json"
  ...
  new_data2=$(echo "$new_data" | jq --arg experiment $exp_name '.save_file.folder = "zz_WikiCS_acc_\($experiment)"')
  # Save the modified JSON data to file_new.json
  echo "$new_data2" > $config_file_mod

  # Run the experiments
  tmux send-keys -t benchmark_WikiCS "
...
tmux new -s benchmark_WikiCS -d

# Send commands to the tmux session
tmux send-keys -t benchmark_WikiCS "cd codes/benchmarking-gnns" C-m
tmux send-keys -t benchmark_WikiCS "conda activate yinka_env" C-m
...
for eta in "${etas[@]}"; do
  tmux send-keys -t benchmark_WikiCS "rm $home/scripts/WikiCS//dcs/pg20/u2034358/codes/benchmarking-gnns/scripts/WikiCS/Experiment/modified_config_$eta.json" C-m
done

# Kill the tmux session
# tmux send-keys "tmux kill-session -t benchmark_WikiCS" C-m
