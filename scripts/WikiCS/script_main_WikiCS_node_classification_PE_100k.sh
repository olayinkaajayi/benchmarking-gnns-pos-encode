...
tmux new -s benchmark_PE -d
# tmux send-keys "source activate benchmark_gnn" C-m
tmux send-keys -t benchmark_PE "cd codes/benchmarking-gnns" C-m
tmux send-keys -t benchmark_PE "conda activate yinka_env" C-m
dataset=WikiCS

tmux send-keys -t benchmark_PE "
CUDA_VISIBLE_DEVICES=$id python $code --dataset $dataset --gpu_id $id --seed $seed0 --config 'configs/WikiCS_node_classification_MoNet_PE_100k.json'
" C-m
