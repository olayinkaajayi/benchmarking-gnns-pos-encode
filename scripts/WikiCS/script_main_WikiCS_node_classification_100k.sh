...
tmux send-keys -t benchmark "cd codes/benchmarking-gnns" C-m
tmux send-keys -t benchmark "conda activate yinka_env" C-m
dataset=WikiCS

tmux send-keys -t benchmark "
CUDA_VISIBLE_DEVICES=$id python $code --dataset $dataset --gpu_id $id --seed $seed0 --config 'configs/WikiCS_node_classification_MoNet_100k.json'
" C-m
