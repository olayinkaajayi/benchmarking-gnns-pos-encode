...
from nets.Pubmed_node_classification.load_net import gnn_model # import GNNs
from data.data import LoadData # import dataset
from train.train_Pubmed_node_classification import train_epoch, evaluate_network # import train functions
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, save_file_params, dirs):
    ...
    if save_file_params['save']:
        ...
        save_float_to_csv(value=[np.mean(np.array(avg_train_acc))*100, np.std(avg_train_acc)*100], filename=os.path.join(save_file_params['folder'],save_file_params['train_file']))
        # Save test metric
        save_float_to_csv(value=[np.mean(np.array(avg_test_acc))*100, np.std(avg_test_acc)*100], filename=os.path.join(save_file_params['folder'],save_file_params['test_file']))
